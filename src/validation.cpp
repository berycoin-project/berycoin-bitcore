...
bool fReindex = false;
bool fTxIndex = false;
bool fAddressIndex = false; // berycoin
...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, bool fCheckDuplicateInputs)
{
    ...
    if (::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > MAX_TRANSACTION_BASE_SIZE ||
        ::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // berycoin
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            for(BerycoinTransaction berycoinTransaction : berycoinTransactions){
                ...
                if(berycoinTransaction.gas() < GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
